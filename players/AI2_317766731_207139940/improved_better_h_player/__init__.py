...
class Player(abstract.AbstractPlayer):
    def __init__(self, setup_time, player_color, time_per_k_turns, k):
        ...
        self.initial_time = time_per_k_turns
    ...
    def get_move(self, game_state, possible_moves):
        ...
        if self.turns_remaining_in_round == 5:
            self.time_for_current_move = self.initial_time / 2
        if self.turns_remaining_in_round == 4:
            self.time_for_current_move = self.initial_time / 4
        if self.turns_remaining_in_round == 3:
            self.time_for_current_move = self.initial_time / 8
        if self.turns_remaining_in_round == 2:
            self.time_for_current_move = self.initial_time / 16
        if self.turns_remaining_in_round == 1:
            self.time_for_current_move = self.initial_time / 32
        current_depth = 1
        prev_alpha = -INFINITY
    ...
    def selective_deepening_criterion(self, state):
        # if self.time_for_current_move / len(state.get_possible_moves()) <= self.time_for_current_move:
        #     return False
        return False
